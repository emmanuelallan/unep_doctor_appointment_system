                ...
                <div class="flex p-6 text-gray-900">
                    <form method="POST" action="{{ route('appointments.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="appointment_date" class="block mt-1 w-full" type="date" name="appointment_date" value="{{ $appointment->appointment_date }}" required autofocus />
                            <x-input-error :messages="$errors->get('appointment_date')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="appointment_time" :value="__('Appointment Time')" />
                            <x-text-input id="appointment_time" class="block mt-1 w-full" type="time" name="appointment_time" value="{{ $appointment->appointment_time }}" required autofocus />
                            ...
                            <div>
                                ...
                                <select name="doctor_id" id="doctor_id" class="block mt-1 w-full">
                                    @foreach ($doctors as $doctor)
                                        <option value="{{ $doctor->id }}" @selected($doctor->id == $appointment->doctor_id)>{{ $doctor->first_name }} {{ $doctor->last_name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="patient_id" id="patient_id" class="block mt-1 w-full">
                                    @foreach ($patients as $patient)
                                        <option value="{{ $patient->id }}" @selected($patient->id == $appointment->doctor_id)>{{ $patient->first_name }} {{ $patient->last_name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="description" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" name="description" required>{{ $appointment->description }}</textarea>
